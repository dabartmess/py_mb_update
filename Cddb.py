...
import getpass, os, re, socket, subprocess, urllib.parse, urllib.request
...
class CddbDiscInfo:
	...
	def __init__(self, discid, artist, album, title, year, genre,
				 tracks: list):
		self.discid = discid
		self.artist = artist
		self.album = album
...
class CddbServer:
	def __init__(self, device = None, cddbServer = 'http://gnudb.gnudb.org/~cddb/cddb.cgi',
		...
		self.reDALBUM = re.compile("DALBUM=(?P<Album>.*)")
		...
		if discid:
			...
			if self.reCode.match(header):
				...
				if self.code == 200: # Exact match
					...
					if m == None:
						print("NO ArtistTitle")

					artist = m.group('Artist').rstrip()
					artist = re.split(r"[\W] ", artist, 0, re.ASCII)
					album = m.group('Title').lstrip()
...
#						album = x.rstrip()
					...
					discs.append( CddbDisc(info[1], info[2], artist,
										   title) )
...
#						if m:
						...
						discs.append( CddbDisc(info[0], info[1], artist,
											   title) )
						line = res.readline().decode('latin-1').rstrip()
	...
	def getDiscInfo(self, disc):
		...
		return CddbDiscInfo(discid, artist, album, title, year, genre, tracks)
